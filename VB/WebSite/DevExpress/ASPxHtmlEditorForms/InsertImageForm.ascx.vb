...
Partial Public Class InsertImageForm
	...
	Protected Function SaveUploadFile() As String
		...
		If HasFile() Then
			...
			Dim pattern As String = "image" & Path.GetExtension(uplImage.UploadedFiles(0).FileName)
			fileName = MapPath(uploadFolder) & pattern
			Try
				' Old file will be overridden as this is an example.
				' In your application use a unique name for files.
				uplImage.UploadedFiles(0).SaveAs(fileName, True)
